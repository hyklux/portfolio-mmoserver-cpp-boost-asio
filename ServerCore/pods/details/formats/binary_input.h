...
namespace pods
{
    namespace details
    {
        ...
        public:
            ...
            Error checkNamePod(const char*) noexcept
            {
                return Error::NoError;
            }
			...
			Error load(SmallString& value)
			{
				uint8_t size = 0;
				PODS_SAFE_CALL(storage_.get(size));
				if (size == 0) return Error::NoError;
				value.str_small.resize(size);
				return storage_.get(const_cast<char*>(value.str_small.data()), size);
			}

			Error load(SmallBinaryChunk& value)
			{
				uint8_t size = 0;
				PODS_SAFE_CALL(storage_.get(size));
                if (size == 0) return Error::NoError;
				value.data.resize(size);
				return storage_.get(value.data.data(), size);
			}
