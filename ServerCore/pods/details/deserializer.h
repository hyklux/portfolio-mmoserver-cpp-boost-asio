...
//#ifdef __PODS_OPTIMZIE__
				PODS_SAFE_CALL(value.serialize(*this, 0));
//#else
...
//#endif
            ...
            {
                const auto result = format_.checkNamePod(name);
            ...
            template <class T>
            Error doProcess(std::optional<T>& value)
            {
                bool hasValue = 0;
                PODS_SAFE_CALL(format_.load(hasValue));
                if (hasValue)
                {
                    T val;
            ...
            Error doProcess(std::unordered_map<K, V, H, KEQ, A>& value)
            {
                uint16_t size = 0;
                PODS_SAFE_CALL(format_.load(size));
            ...
            Error doProcess(std::unordered_set<K, H, KEQ, A>& value)
            {
                uint8_t size = 0;
                PODS_SAFE_CALL(format_.load(size));
            ...
            Error doProcess(std::set<K, KEQ, A>& value)
            {
                pods::Size size = 0;
                PODS_SAFE_CALL(format_.load(size));

                for (pods::Size i = 0; i < size; ++i)
                {
                    K elem;
                    PODS_SAFE_CALL(doProcess(elem));
                    value.insert(elem);
			...
			Error doProcess(std::multiset<K, C, A>& value)
			{
                uint16_t size = 0;
				PODS_SAFE_CALL(format_.load(size));
			...
			Error doProcess(SmallString& value)
			{
				return format_.load(value);
			}

            Error doProcess(SmallBinaryChunk& value)
			{
				return format_.load(value);
			}
            ...
            Error doProcess(std::vector<T, A>& value)
            {
                return loadSequenceContainer<T>(value);
            }

			template <class T>
			Error doProcess(SmallVector<T>& value)
			{
				return loadSmallSequenceContainer<T>(value);
			}
			...
			template <class T, class Container>
			Error loadSmallSequenceContainer(Container& value)
			{
				return loadSmallArray<T>(
					[&](uint8_t size)
					{
						value.vec.resize(size);
						return Error::NoError;
					},
            ...
            Error doProcess(std::pair<Key, Val>& value)
            {
                ...
                PODS_SAFE_CALL(format_.checkNamePod(PODS_KEY));
                PODS_SAFE_CALL(doProcess(value.first));

                PODS_SAFE_CALL(format_.checkNamePod(PODS_VAL));
            ...
            Error doProcess(std::map<K, V, H, A>& value)
            {
                return loadMap<K, V>(
...
//#endif
			...
			template <class T, class Init, class GetIterator, typename std::enable_if<IsPodsSerializable<T>::value, int>::type = 0>
			Error loadSmallArray(Init init, GetIterator getIterator)
			{
				PODS_SAFE_CALL(format_.startObject());
...
//#endif
				...
				uint8_t size = 0;
				PODS_SAFE_CALL(format_.startSmallArray(size));
            ...
            {
                PODS_SAFE_CALL(format_.checkNamePod(name));
