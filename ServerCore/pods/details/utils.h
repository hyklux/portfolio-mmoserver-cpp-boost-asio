...
namespace pods
{
    namespace details
    {
        ...
        {
        private:
            static void podcheck(...);

            template <class U>
            static decltype(U::version()) podcheck(U*);

        public:
            static constexpr bool value =
                std::is_same<Version, decltype(IsPodsSerializableImpl<T>::podcheck(static_cast<std::remove_reference_t<T>*>(nullptr)))>::value;
        };
